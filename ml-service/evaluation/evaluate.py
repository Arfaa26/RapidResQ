...
def main():
    ...
    directory = Path(args.model_dir or f'models/{args.kind}')
    ...
    if classifier.status()['status'] != 'ready':
        raise ValueError('Model requires training. No evaluation numbers will be generated.')
    meta = classifier.metadata if args.kind == 'image' else classifier.bundle['metadata']
    for row in test:
        if row['content_hash'] in meta['developmentContentHashes'] or row['group_hash'] in meta['developmentGroupHashes']:
    ...
    output = {'modelVersion': meta['modelVersion'], 'algorithm': meta['algorithm'], 'split': 'test',
              ...
              'abstentionCount': sum(p['status'] != 'ready' for p in predictions)}
    (directory / 'evaluation.json').write_text(json.dumps(output, indent=2, allow_nan=False), encoding='utf-8')
